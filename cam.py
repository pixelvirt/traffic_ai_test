...
try:
    ...
    while True:
        ...
        if frame_count % (CAPTURE_INTERVAL * fps) == 0:
            ...
            success_capture, img_capture = cap.read()
            if not success_capture:
                print("Can't receive frame (stream end?). Exiting ...")
                break

            results = YOLO_MODEL_LARGE(img_capture, stream=True, conf=CONFIDENCE_THRESHOLD)
            for r in results:
                detected_classes = []
                boxes = r.boxes
                for box in boxes:
                    ...
                    cv2.rectangle(img_capture, (x1, y1), (x2, y2), (255, 0, 0), 1)
                    cv2.putText(img_capture, f"{conf} {coco_class_list[cls]}", (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                ...
                for class_name, count in counter.items():
                    cv2.putText(img_capture, f"{class_name}: {count}", (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                    text_y += 15

                cv2.imwrite(f"{OUTPUT_FOLDER}/{frame_count}.jpg", img_capture)
